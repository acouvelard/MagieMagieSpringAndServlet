...
public interface PartieDAOCrud extends CrudRepository<Partie, Long>{
    ...
    @Query("SELECT p FROM Partie p "
            + "EXCEPT SELECT p FROM Partie p JOIN p.joueurs j "
            + "WHERE j.etat = atos.magie_magie.entity.Joueur.EtatJoueur.A_LA_MAIN"
            + "OR j.etat = atos.magie_magie.entity.Joueur.EtatJoueur.GAGNE")
